        ...
        <div className="min-h-screen flex flex-col bg-gray-100">
            ...
            <main className="container mx-auto px-4 py-6 flex-1">
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">VIP Ranks</h1>
                    <p className="text-gray-500">Compare ranks and choose the best one for you</p>
                </div>

                {products.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-xl border">
                        <Crown className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                        <p className="text-gray-500">No VIP ranks available yet</p>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b">
                                    <th className="text-left py-4 px-6 font-medium text-gray-500 min-w-[200px]">Feature</th>
                                            ...
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <div>
                                                    {product.comparePrice && (
                                                        <span className="text-xs text-gray-400 line-through mr-1">{formatPrice(product.comparePrice)}</span>
                                                    )}
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {products.map((product, i) => (
                            <div
                                key={product.id}
                                className={`bg-white rounded-xl border p-6 text-center relative ${i === Math.floor(products.length / 2) ? "ring-2 ring-blue-500 scale-105" : ""}`}
                            >
                                ...
                                <div className="mb-4">
                                    {product.comparePrice && (
                                        <span className="text-gray-400 line-through text-sm mr-2">{formatPrice(product.comparePrice)}</span>
                                    )}
