    ...
    return (
        <>
            <div className="mb-8">
                ...
                <p className="text-muted-foreground">{apps.filter(a => a.status === "pending").length} pending review</p>
            </div>
            ...
            <div className="flex gap-2 mb-6">
                {["", "pending", "accepted", "rejected"].map((s) => (
                    <Button key={s} variant={filter === s ? "default" : "outline"} size="sm" onClick={() => setFilter(s)}>
                        ...
                        {s === "pending" && <span className="ml-1 text-xs">({apps.filter(a => a.status === "pending").length})</span>}
                    </Button>
                ))}
            </div>
                    ...
                    {filtered.map((app) => (
                        <Card key={app.id}>
                            <CardContent className="p-5">
                                ...
                                {app.adminNote && (
                                    <p className="text-sm text-muted-foreground mb-3">Admin note: {app.adminNote}</p>
                                )}
