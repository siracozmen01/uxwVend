    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-100">
            ...
            <main className="container mx-auto px-4 py-6 flex-1 flex flex-col items-center">
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Wheel of Fortune</h1>
                <p className="text-gray-500 mb-8">Spin the wheel for a chance to win prizes! One free spin per day.</p>

                {loading ? (
                    <Loader2 className="w-8 h-8 animate-spin text-gray-400" />
                ) : prizes.length === 0 ? (
                    <Card><CardContent className="py-12 text-center text-gray-500">No prizes configured yet</CardContent></Card>
                ) : (
                    <div className="flex flex-col items-center gap-6">
                            ...
                            <Card className={`w-full max-w-sm ${result.type === "error" ? "border-red-200" : "border-green-200"}`}>
                                <CardContent className="p-6 text-center">
                                    {result.type === "error" ? (
                                        ...
                                        <p className="text-gray-500">Better luck next time! You got: <strong>{result.name}</strong></p>
                                    ) : (
                                        <div>
                                            <p className="text-2xl mb-1">🎉</p>
                                            <p className="font-bold text-gray-900 text-lg">You won: {result.name}!</p>
                                            {result.value > 0 && (
                        ...
                        <div className="w-full max-w-sm">
                            <h3 className="text-sm font-medium text-gray-500 mb-2">Available Prizes</h3>
                            <div className="space-y-1">
                                {prizes.map((p) => (
                                    <div key={p.id} className="flex items-center gap-2 text-sm">
                                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: p.color }} />
                                        <span className="text-gray-700">{p.name}</span>
                                        {p.value > 0 && <span className="text-gray-400 text-xs">({p.type === "credits" ? `${p.value} credits` : `$${p.value}`})</span>}
                                    </div>
                                ))}
                            </div>
                        </div>
